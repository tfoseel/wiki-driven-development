...
- 산출물: `tests/e2e/reschedule-booking.spec.ts`, `tests/e2e/cancel-booking.spec.ts`
...
## 화면 트리
- [[screens/booking-detail]]: 활성 예약과 가능한 후속 행동을 확인한다.

  ![예약 상세 QA 통과 화면](../자료/스크린샷/화면/booking-detail.png)

  - 일정 변경 성공 -> [[screens/booking-detail]] 갱신된 시간 슬롯 상태를 보여준다.
  - 취소 성공 -> [[screens/booking-detail]] 취소된 예약 상태를 보여준다.
  - 정책 차단 -> [[screens/booking-detail]] 정책 사유와 유지된 예약 상태를 보여준다.
  - 알 수 없는 예약 id -> [[screens/booking-detail]] 찾을 수 없음 상태를 보여준다.
