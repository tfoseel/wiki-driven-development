...
screenshots:
  - path: wiki/자료/스크린샷/화면/booking-complete.png
    alt: 예약 완료 QA 통과 화면
    route: /bookings/complete?bookingId=booking-confirmed-001
mockups:
  - path: wiki/자료/목업/화면/booking-complete.html
    alt: 예약 완료 기획 목업
    status: implemented
verify:
...
- 노드: `screens/booking-complete`
- 타입: `screen`
- 의존: [[models/booking]], [[design/design-system]]
- 구현: `src/app/bookings/complete/page.tsx`
- 검증 파일: `tests/e2e/create-booking.spec.ts`
- 산출물: `src/app/bookings/complete/_components/booking-complete-screen.tsx`
- 스크린샷: `wiki/자료/스크린샷/화면/booking-complete.png` (/bookings/complete?bookingId=booking-confirmed-001)
- 검증 명령: `npm run e2e -- create-booking`

</details>

## 화면 증거
![예약 완료 QA 통과 화면](../자료/스크린샷/화면/booking-complete.png)

기획 목업: [예약 완료 기획 목업](../자료/목업/화면/booking-complete.html)
...
## 사용자 행동
- [[screens/booking-detail]]로 이동한다.
- [[screens/service-list]]로 돌아간다.
- 예약 내용을 `.ics` 파일로 내려받아 캘린더에 추가한다.

## 기획 목업
- `wiki/자료/목업/화면/booking-complete.html`
...
## 내비게이션과 전달
- [[screens/booking-detail]]로 `bookingId`를 전달한다.

## 독립 QA
- given seed-booking-complete-normal / when 화면 로드 / then 완료 상태 배지와 예약 요약 패널이 보인다
- given seed-booking-complete-normal / when 화면 로드 / then 캘린더에 추가 링크가 보인다
...
- given seed-booking-complete-with-note / when 화면 로드 / then 요청사항이 보인다
- given seed-booking-complete-with-photo / when 화면 로드 / then 첨부 사진이 보인다
- given seed-booking-complete-missing-id / when 화면 로드 / then 오류가 보인다
- given seed-booking-complete-missing-id / when 화면 로드 / then 캘린더에 추가 링크가 보이지 않는다
