...
- 새 예약 화면의 폼은 시간 선택, 이름, 이메일, 요청사항, 제출 순서로 흐른다.
- 예약 완료 화면은 서비스와 시간을 먼저 확인시키고, 요청사항이 있으면 함께 보여준 뒤 예약 관리 링크를 제공한다.
- 예약 상세 화면은 상태, 서비스, 시간, 요청사항, 가능한 행동을 분리해서 보여준다.
...
- `/bookings/booking-confirmed-001`에서 예약 상태, 요청사항, 가능한 행동이 보인다.
